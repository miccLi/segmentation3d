...
from vnet_with_2dunet import vnet
...
if dataset == 'VISCERAL_multiple':
    data_train_path = '/media/xz6214/4276F10376F0F90D/datasets/VISCERAL/multiple_2fold_%d_%d_%d/train_%d/' % (
        vol_size[2], vol_size[1], vol_size[0], cross_validation)
    data_test_path = '/media/xz6214/4276F10376F0F90D/datasets/VISCERAL/multiple_2fold_%d_%d_%d/test_%d/' % (
        vol_size[2], vol_size[1], vol_size[0], cross_validation)
...
prediction, logits = vnet(vol_in, labels, feature_channels=feature_channels, output_channels=output_channels)
...
for lr_train in [0.1, 0.05, 0.01, 0.005]:
    ...
    save_path = '/media/xz6214/4276F10376F0F90D/trained_model/%s/%d_%d_%d/vnet_with_2dunet/model_lr_%f_crossval_%s/' % (
    ...
    with tf.Session(config=config) as sess:
        ...
        for step in range((train_num // batch_size) * epoch):
            ...
            for image_id in image_id_list:
                ...
                lable_np = np.reshape(lable_np,
                                      (1, lable_np.shape[0], lable_np.shape[1], lable_np.shape[2], lable_np.shape[3]))
...
def tversky_loss(gt, prob, alpha=0.5, beta=0.5):
    p0 = prob
    p1 = tf.ones(gt.shape) - prob
    g0 = gt
    g1 = tf.ones(gt.shape) - gt
    ...
    return tf.cast(gt.shape[-1], dtype=tf.float32) - t
